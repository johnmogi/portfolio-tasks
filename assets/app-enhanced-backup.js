...
$(document).ready(function() {
    ...
    function startBackgroundTimer() {
        ...
        // Check every 15 minutes for automatic logging
        activeTimer = setInterval(() => {
            const trackingTask = tasks.find(task => task.isTracking);
            if (trackingTask && trackingTask.trackingStartTime) {
                const elapsedMinutes = Math.floor((Date.now() - trackingTask.trackingStartTime) / (1000 * 60));
                ...
                if (elapsedMinutes % 15 === 0 && elapsedMinutes > 0) {
                    ...
                    trackingTask.timeLogs.push(logEntry);
                    trackingTask.timeSpent += 15;

                    console.log(`⏱️ Auto-logged 15 minutes for task: ${trackingTask.title}`);
                    saveTasks();
                    refreshCurrentView();
                }
            }
        ...
        console.log('🔄 Background timer started - auto-logging every 15 minutes');
    }
    ...
    function startTimer(taskId) {
        ...
        task.trackingStartTime = Date.now(); // Set to actual current time
    ...
    function toggleTimer(taskId) {
        ...
        if (task.isTracking) {
            ...
            console.log(`⏱️ Timer stopped - ${elapsed} minutes logged for task: ${task.title}`);
            showToast(`Timer stopped - ${elapsed} minutes logged`, 'info');
        } else {
            // Start timer
            const now = Date.now();
            task.isTracking = true;
            task.trackingStartTime = now; // Set to actual current time
                ...
                rowGroup: {
                    ...
                    startRender: function(rows, group) {
                        ...
                        return $('<tr class="bg-blue-50 dark:bg-blue-900/20">')
                            .append('<td colspan="7" class="px-6 py-3 text-left text-sm font-medium text-blue-800 dark:text-blue-200"><strong>' + group + '</strong></td>')
                            .get(0);
                    }
                }
            });
